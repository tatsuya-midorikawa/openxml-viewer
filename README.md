...
- [.NET SDK](https://dotnet.microsoft.com/) `10.0` 以降（F# / Fable のビルドに使用）
...
npm run watch          # 例: dotnet fable watch src --outDir build --test:MSBuildCracker

# 単発ビルド
npm run build          # 例: dotnet fable src --outDir build --test:MSBuildCracker && esbuild
```
